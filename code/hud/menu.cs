...
public class MembersBase:Panel{
    public MembersBase(){
        foreach(int i in (Local.Pawn as Citizen).org.Members){
...
public class Member:Panel{
    ...
    public Member(int cid){
        bool incache = false;
        foreach(Profile p in (Game.Current as RoleplayGame).profilecache){
        ...
        if (incache == false){
            foreach (Profile p in (Game.Current as RoleplayGame).AllProfiles){
                            if(p.CharacterID == cid){
                incache = true;
                characterid = p.CharacterID;
                name = p.Name;
